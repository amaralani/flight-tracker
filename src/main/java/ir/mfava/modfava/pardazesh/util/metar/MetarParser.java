 ...
 */
package ir.mfava.modfava.pardazesh.util.metar;
...
public class MetarParser {
     ...
     *             Unable to parse report
     */
    public static Metar parseTAFReport(String metarString)
            throws MetarParseException {
        try {
            ...
            log.debug("TAFParser: instantiated");

            if (metarString == null) {
                throw new MetarParseException("empty metar data");
            }

            log.debug("TAFParser: raw: " + metarString);
            ...
            try {
                ...
                log
                        .error("TAFParser: error spliting metar data on whitespace: "
                                + e);
            ...
            log.debug("TAFParser: have '" + numTokens + "' tokens");
            ...
            log.debug("TAFParser: stationID: " + metar.getStationID());

            log.debug("TAFParser: processing ((String)tokens.get(" + index
                    + "))=" + tokens.get(index));
            ...
            if (((String) tokens.get(index)).endsWith("Z")) {
                ...
                log.debug("TAFParser: date: " + metar.getDate());
            } else {
                log.debug("TAFParser: date: no date found");
            ...
            if (tokens.get(index)
                ...
                log.debug("TAFParser: report modifier: "
                        + metar.getReportModifier());
            } else {
                log.debug("TAFParser: no report modifier");
            }

            log.debug("TAFParser: processing ((String)tokens.get(" + index
                    + "))=" + tokens.get(index));

            // TODO : change validation time here
            if (index < numTokens - 1) {
                index++;
            }
            ...
            if (temp.endsWith("KT") || temp.endsWith("KTS") ||
                ...
                if (temp.endsWith("KT") ||
                    ...
                    log.debug("TAFParser: wind speed in knots");
                    windInKnots = true;
                } else {
                    log.debug("TAFParser: wind speed in meters per second");
                }
                ...
                if (!((String) tokens.get(index)).substring(0, 3).equals("VRB")) {
                    ...
                    log.debug("TAFParser: variable wind direction <= 6 knots");
                ...
                if(!metar.getWindDirectionIsVariable() &&
                    ...
                    log.debug("TAFParser: have three-digit wind speed");
                    if (windInKnots) {
                    ...
                    log.debug("TAFParser: have two-digit wind speed");
                    if (windInKnots) {
                        metar.setWindSpeed(new Float(((String) tokens
                ...
                if (temp.length() > pos && temp.charAt(pos) == 'G') {
                    // we have wind gusts
                    log.debug("TAFParser: wind gusts");
                    pos++;
                    ...
                    try {
                        if (matcher.matches(temp, new Perl5Compiler()
                            ...
                            // have three-digit wind speed
                            log
                                    .debug("TAFParser: have three-digit wind speed");
                            if (windInKnots) {
                        ...
                        log.error("TAFParser: error matching wind gusts: "
                                + e);
                    }
                } else {
                    // we don't have gusts
                    log.debug("TAFParser: no gusts");
                }

                log.debug("TAFParser: wind direction: "
                        + metar.getWindDirection());
                if (windInKnots) {
                    log.debug("TAFParser: wind speed: "
                ...
                log.debug("TAFParser: processing ((String)tokens.get("
                ...
                try {
                    if (matcher.matches(temp, new Perl5Compiler()
                        ...
                        log.debug("TAFParser: variable wind direction min: "
                                + metar.getWindDirectionMin());
                        log.debug("TAFParser: variable wind direction max: "
                    ...
                    log
                            .error("TAFParser: error matching variable wind speed: "
                                    + e);
                }
            } else {
                // unexpected token...should have been wind speed
                log.debug("TAFParser: wind speed: not found");
            }
            ...
            if (tokens.get(index)
                ...
                log.debug("TAFParser: visibility");
                ...
                if (token.startsWith("M")) {
                    log.debug("TAFParser: visibility: less than");
                    isLessThan = true;
                ...
                if (!fraction.equals("")) {
                    ...
                    try {
                        ...
                        log
                                .error("TAFParser: error spliting fraction on /: "
                                        + e);
                        throw new MetarParseException(
                ...
                if (utility.match("/^M?\\d+(N|NE|E|SE|S|SW|W|NW)?$/", token)) {
                    ...
                    log.debug("TAFParser: visibility: not found");
                }
            }
            ...
            while (((String) tokens.get(index)).startsWith("R")) {
                ...
                log.debug("TAFParser: RVR runway number: "
                ...
                if (((String) tokens.get(index)).charAt(pos) != '/') {
                    ...
                    log.debug("TAFParser: RVR runway approach direction: "
                ...
                switch (((String) tokens.get(index)).charAt(pos)) {
                    ...
                    case 'M': // above 6000ft
                        runwayVisualRange.setReportableModifier(((String) tokens
                                .get(index)).charAt(pos));
                        log.debug("TAFParser: RVR modifier: "
                                + ((String) tokens.get(index)).charAt(pos));
                        pos++;
                }
                runwayVisualRange.setLowestReportable(new Integer(
                        ...
                        .intValue());
                log.debug("TAFParser: RVR lowest reportable: "
                ...
                if (((String) tokens.get(index)).charAt(pos) == 'V') {
                    ...
                    log.debug("TAFParser: RVR highest reportable: "
            ...
            while (((String) tokens.get(index))
                ...
                log.debug("TAFParser: found weather groups");
                ...
                if (((String) tokens.get(index))
                    ...
                    log.debug("TAFParser: weather group: intensity: "
                            + ((String) tokens.get(index)).charAt(0));
                    pos++;
                } else {
                    log
                            .debug("TAFParser: weather group: intensity: moderate");
                }
                ...
                if (((String) tokens.get(index)).substring(pos, pos + 2)
                    ...
                    weatherCondition.setDescriptor(((String) tokens.get(index))
                            .substring(pos, pos + 2));
                    log.debug("TAFParser: weather group: descriptor: "
                            + ((String) tokens.get(index)).substring(pos,
                            pos + 2));
                    pos += 2;
                } else {
                    log
                            .debug("TAFParser: weather group: descriptor: no descriptor");
                }
                ...
                if (((String) tokens.get(index)).substring(pos, pos + 2)
                    ...
                    weatherCondition.setPhenomena(((String) tokens.get(index))
                            .substring(pos, pos + 2));
                    log.debug("TAFParser: weather group: phenomena: "
                            + ((String) tokens.get(index)).substring(pos,
                            pos + 2));
                    ...
                    log.debug("TAFParser: weather group: no phenomena");
                }
            ...
            while (((String) tokens.get(index))
                ...
                log.debug("TAFParser: found sky conditions");
                ...
                if (((String) tokens.get(index))
                    ...
                    if (((String) tokens.get(index)).length() > 6) {
                        ...
                        skyCondition.setModifier(((String) tokens.get(index))
                                .substring(6, ((String) tokens.get(index))
                                        .length()));
                        log.debug("TAFParser: sky condition: modifier: "
                    ...
                    skyCondition.setContraction(((String) tokens.get(index))
                            .substring(0, 3));
                    log.debug("TAFParser: sky condition: clear: "
                            + ((String) tokens.get(index)).substring(0, 3));
                } else if (((String) tokens.get(index))
                        .startsWith(MetarConstants.METAR_VERTICAL_VISIBILITY)) {
                    skyCondition.setContraction(((String) tokens.get(index))
                    ...
                    skyCondition.setContraction(((String) tokens.get(index))
                            .substring(0, 3));
                    log
                            .debug("TAFParser: sky condition: No Significant Clouds: "
                                    + ((String) tokens.get(index)).substring(0,
                                    3));
                } else {
                    log
                            .debug("TAFParser: sky condition: couldn't determine which");
                }
            ...
            if (((String) tokens.get(index)).indexOf("/") != -1) {
                log.debug("TAFParser: found temperature");
                ArrayList temps = new ArrayList();

                try {
                    utility.split(temps, "/\\//", ((String) tokens.get(index)));
                } catch (MalformedPerl5PatternException e) {
                    log.error("TAFParser: error spliting temperature on /: "
                            + e);
                    throw new MetarParseException(
                ...
                if(temperature != null)
                    log.debug("TAFParser: temperature: " + temperature + " C, "
                ...
                if(dewPoint != null)
                    log.debug("TAFParser: dew point: " + dewPoint + " C, "
                ...
                log.debug("TAFParser: temperature/dew point not found");
                metar.setTemperature(null);
                metar.setDewPoint(null);
            }
            ...
            if (((String) tokens.get(index)).startsWith("A")) {
                ...
                log.debug("TAFParser: pressure: " + metar.getPressure()
                        + " Hg");
            ...
            // Measured in hecto Pascal (HPa), 1 Hpa = 1 mB(millibar)
                ...
                log.debug("TAFParser: pressure: " + pressure + " hPa");
                ...
                log.debug("TAFParser: pressure not found");
            }
            ...
            if (!tokens.get(index)
                ...
                // we have no remarks
                log.debug("TAFParser: we have no remarks");
            } else {
                log.debug("TAFParser: we have remarks");
                index++;
            }
            ...
            if(index < numTokens &&
                ...
                log.debug("TAFParser: processing \"BECMG\" section");
            ...
            while (index < numTokens) {
                ...
                try {
                    if (matcher.matches(temp, new Perl5Compiler()
                            .compile("T\\d{8}"))) {
                        log.debug("TAFParser: found detailed temp: "
                                + tokens.get(index));
                        // we have a sub-zero temperature
                        ...
                        log
                                .debug("TAFParser: temperature precise: "
                                        + temperaturePrecise
                                        + " C, "
                                        + (temperaturePrecise.floatValue() * 9 / 5 + 32)
                                        + " F");
                        log.debug("TAFParser: dew point precise: "
                                + dewPointPrecise + " C, "
                    ...
                    log
                            .error("TAFParser: error matching additional remarks: "
                                    + e);
                }
            ...
            log.debug("TAFParser: done processing metar data");
